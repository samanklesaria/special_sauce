...
# ╔═╡ eb0e0e02-89a4-443b-b4fd-c49b98710614
using KernelFunctions, LinearAlgebra, SparseArrays, AbstractGPs

# ╔═╡ 9eff7071-ccb6-4f57-9672-d9a0e9313fe4
using CairoMakie, AbstractGPsMakie

# ╔═╡ 09a3b185-8ea7-4f0c-b5c8-08f0a4385591
using ForwardDiff, ParameterHandling, Optim, Zygote

# ╔═╡ a2a30ab2-cc63-11ee-0cb4-bb37f4377d20
md"## Nearest Neighbor Gaussian Processes"

# ╔═╡ 70ef5870-70c3-47ca-82d0-f912b3b2597d
md"""
...
# ╔═╡ d6a80be2-4ffa-4fc7-b958-4c39fdb9dc4d
md"## Implementation"

# ╔═╡ 4cafb1dd-ba39-41b5-9abd-d7d8ee84ce24
md"We assume that `pts` are in order, and that each point is independent of all the previous ones given the previous `k`."

# ╔═╡ 8f235339-c5d1-4e45-975f-aeddf5181b6b
...
# ╔═╡ 67f482a7-bb6a-4421-949b-d6da5a62516a
md"To the understand the form of the B matrix described above more clearly, consider its form in a 2-nearest neighbor Gaussian Process."

# ╔═╡ 8ff4fc62-71db-4e3e-83f0-7160bfde6d5a
pts = [1., 2., 3.5, 4.2, 5.9, 8.];

# ╔═╡ 30fca9b6-644a-4eba-8c5d-c84725559945
kern = SqExponentialKernel();

# ╔═╡ 68019c06-5bce-495f-8f46-654ba1987f7c
B = make_B(pts, 2, kern)

# ╔═╡ a613fe24-54fe-4c9e-b3a8-4b20aef99d3f
md"The $F$ matrix can be constructed analogously."

# ╔═╡ 77a66373-aae9-4225-a44a-cf9e804221f7
...
# ╔═╡ 7305ae9c-6801-480c-b30c-82fd59919166
F = make_F(pts, 2, kern)

# ╔═╡ 3d368151-b1aa-48f9-b44f-907c7a827ae2
md"The associated covariance matrix has the form $(I-B)^{-1}F(I-B)^{-1}$. We can compare this approximation to the full (non nearest-neighbor) covariance matrix." 

# ╔═╡ be345d2e-c6dd-465d-bf19-fbbfba8ef96d
L = (I - B) \ sqrt(F)

# ╔═╡ baf7fd5f-e894-41a2-89c0-7582d1d4322f
L * L'

# ╔═╡ f5389711-e5dc-43e7-b355-722e247e26f7
kernelmatrix(kern, pts)

# ╔═╡ c194d852-157b-47b4-9c27-b32078b18804
md"The two are pretty close!"

# ╔═╡ debbd505-6084-46c8-8dc2-a17705bb81c5
md"""
## Interface
To make this usable with Julia's AbstractGPs library, we'll add a new method for the `posterior` function.
"""

# ╔═╡ 646a3e73-9de5-4b88-b40b-c0e798d8b201
struct NearestNeighbors
	k::Int
end

# ╔═╡ b35abd03-08ce-4a6f-a5aa-c40b9e385df1
struct InvRoot{A}
	U::A
end

# ╔═╡ 1e69ac7b-fee5-4445-a218-ed90fbd71e3d
AbstractGPs.diag_Xt_invA_X(A::InvRoot, X::AbstractVecOrMat) = AbstractGPs.diag_At_A(A.U' * X)

# ╔═╡ a5c3d895-85fb-4d5f-a116-b594d32a4510
...
# ╔═╡ 2d5defb8-9d38-4262-9028-e81a0fa3ba91
md"Here's how we use it:"

# ╔═╡ e724a0fb-ac61-455d-a4f3-d3e92e3d9f92
y = sin.(pts);

# ╔═╡ e7589728-62f0-4a13-98d5-6b7eba409500
fx = GP(kern)(pts, 0.0);

# ╔═╡ 66bd5952-f45f-4966-a84f-b566c3b366cc
md"Note that the nearest neighbor approximation requires a noise-free GP."

# ╔═╡ 167f7911-c0c8-404c-80eb-7dcc1ac290a8
post = posterior(NearestNeighbors(5), fx, y);

# ╔═╡ c47985a9-de1b-4f15-9228-3d23477c4c48
plot(1:0.01:8, post)

# ╔═╡ 9db399e1-5d38-4c83-81a5-0b02bdf8ca33
md"""
...
# ╔═╡ 9917296d-684a-4a8a-b277-1cd2e33d7044
Zygote.gradient(objective, flat_initial_params)

# ╔═╡ c5a6592f-524f-488e-87ec-eca8bdc810dc
md"Zygote handles autodifferentiation without issues!"

# ╔═╡ 2f65cadc-1bde-4ac5-be24-975c1d7b7f62
...
# ╔═╡ 1fd68981-c45e-447f-97e2-3fd44928a0c9
md"With optimized parameters, we get much less uncertainty in our predictions."

# ╔═╡ 7e23c9b7-0696-461e-ad16-19dc75022d02
...
manifest_format = "2.0"
project_hash = "5e54f95b509b7ab46e6f2e9a0e3eadecda9b45a4"
