...
# ╔═╡ a9cee4eb-da63-4a88-bc39-5e5f4141a824
const Dist = Tuple{SVector{2, Float64}, SMatrix{2,2, Float64}}

# ╔═╡ 2f6e7b32-2d3c-421b-a9a3-7187695ea8af
sample(d::Dist) = rand(Distributions.MvNormal(d[1], d[2]))

# ╔═╡ e9765535-ea10-44d6-a651-20136b5e2aa8
struct Particle
...
# ╔═╡ 3ecad9ba-42d4-46af-a202-f771ff055a5c
const k = 6; # Number of obstacles

# ╔═╡ 0a2fbcf5-8e6a-40cc-9c87-f866117c2f66
...
# ╔═╡ 5e81ba4a-6494-43af-b791-8674cc0d5390
const n = 100; # Number of particles

# ╔═╡ 19a5dd73-95be-4e2b-aa22-74aebf3137f9
particles = init_particles(n);

# ╔═╡ 1d42ecab-acea-4824-b420-c9740b0328c0
md"""
...
# ╔═╡ 4127fb2b-0c4f-4a21-8e75-befabc427202
md"""
At each step $t$ in time, our algorithm will do the following:
1. Resample the particles in proportion to their importance weights. This effectively turns our samples from $q_t$ into samples from $\pi_t$. 
2. Extend each particle, turning the samples from $\pi_t$ into samples from $q_{t+1}$. This requires sampling from $p(x_{t+1}^i | x_t^i, z_{1:t})$, computing the importance weight $p(z_{t+1} | x_T, z_{1:t})$, and updating the associated distribution $p(m | x_{1:t}, z_{1:t})$.

At $t=0$, the proposal and target distributions are the same so we don't need to resample. 
"""

# ╔═╡ 8abc969f-ff18-4b1d-8cae-006932ceb13f
md"""
# Resampling
If `w` is our vector of weights, we want to sample $k$ elements of `w` in proportion to their values. The most obvious way to do this is with inverse transform sampling. Let `b = cumsum(w)`. For each random sample, we can pick a uniform $u_i$ between $0$ and $W$, which corresponds to `searchsortedfirst(b, u_i)`.
...
# ╔═╡ 3b681152-1867-4586-9c14-627cc1e32eed
md"""
# Proposing
...
# ╔═╡ 6a71baad-a6a9-4539-bd00-bc89e9d8a063
begin
ΣA = Eye(2,2)
ΛA = inv(ΣA)
ΣZ =  (1/6) * Eye(2,2)
...
# ╔═╡ 61391e7d-6f0f-47d8-b974-7c211e6574f8
post_msg_m(x, z) = (ΛZ \ (z - x), ΛZ)

# ╔═╡ 38699615-636f-4b3a-9947-f39767de30f4
function stdform((mu, var))
	(var \ mu, inv(var))
end

# ╔═╡ 9970d580-b5c8-41f2-b7db-b874c77c05c7
function meanform((prec_mu, prec))
	(prec \ prec_mu, inv(prec))
end

# ╔═╡ 05d3a5e1-c3f4-4c1e-804d-9e5175ec3acc
md"""
# Fake Data
Here we generate the unknown, true trajectory that the robot takes by following our dynamics model.
"""

# ╔═╡ 889310ac-0d91-41a1-8b30-87f33d0cdf67
md"We'll also set up the true but unknown obstacle locations."

# ╔═╡ 9da8e15b-fa4d-4e51-bfe9-26aa75ea9cab
true_ms = 10 * randn(2, k);

# ╔═╡ 0cc0ea07-43f2-423f-b8a7-6bc43a0af0e4
md"The displacement observations the robot sees along its trajectory look like this:"

# ╔═╡ 26990dc4-d538-4a9b-8114-b299350d96bc
md"To show the direction of time, lines will get fainter with time."

# ╔═╡ 484fbd95-c7f4-49b9-9df7-e562e9225404
md"# Simulating the Algorithm"

# ╔═╡ 157e6c23-954e-49f1-9e65-d70714d730d6
Base.:+(a::Tuple, b::Tuple) = a .+ b # So we can add distribution parameters

# ╔═╡ 2abeb034-4a52-49f7-bc72-ea299026670b
...
# ╔═╡ 356e0891-a08b-4a1a-8851-fa613df5025f
function post_msg_x(m, z)
	ΣBZ = m[2] + ΣZ
	(ΣBZ \ (m[1] - z), inv(ΣBZ))
end

# ╔═╡ 384beddc-23d9-4a65-bd89-029b9842300c
function importance(m, z, xmean)
	logpdf(Distributions.MvNormal(m[1] - xmean, m[2] + ΣZ), z)
end

# ╔═╡ 2085dce2-724a-452c-95d5-cadab471d3c1
...
# ╔═╡ bfc2e3ba-28e0-416b-9651-19291ce1820c
...
# ╔═╡ c05193b0-3f20-4e82-8579-b45717e7a543
timealpha = reverse(0.1 .+ ((1:size(z, 3))./size(z, 3)));

# ╔═╡ 8de99c36-de4f-4f80-a442-b95e283d8e11
begin
...
# ╔═╡ bea9d1cb-36fe-4682-be3d-576bea92d053
sz = map(SVector{2}, eachslice(z; dims=(3,2))); # Rewrites z as a matrix of SVec

# ╔═╡ c2ed6ff5-8539-4145-905e-d0e8d109deb2
steps = accumulate(step, eachslice(sz;dims=1); init=particles);

# ╔═╡ 5928d5a8-3123-4d02-a378-842200b33c74
predicted = SVector{2, Float64}[mean(p-> p.x, s) for s in steps];

# ╔═╡ 80e1d8da-6aef-4350-8a47-cc3b09e07eca
flat_pred = flatview(ArrayOfSimilarArrays(predicted));

# ╔═╡ fc88e5e9-5642-46bf-991f-4c6112814c20
begin
plot(true_xs[1, :], true_xs[2, :], label="robot path")
plot!(flat_pred[1,:], flat_pred[2,:], label="predictions", alpha=timealpha)
end

# ╔═╡ 5f4a9b52-4804-4f80-be46-ab5dbcc24616
md"Even though the robot's trajectory is noisy, the observations allow us to track it quite accurately. "
...
manifest_format = "2.0"
project_hash = "4791d36f92dcd5760be04bde1666fe96cc660afa"
